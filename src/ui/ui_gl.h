...
typedef struct {
    ...
    int       batch_floats;  /* floats consumed this frame; reset by ui_gl_begin */
} ui_gl_t;
...
/* Begin a frame: cache the framebuffer size and pointer state, rewind the
   ...
   origin); they drive ui_gl_button hover/click. */
