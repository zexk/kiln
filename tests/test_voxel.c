...
Test(voxel, empty_chunk_generates_no_geometry) {
    ...
    KvMesh m;
    kv_mesh_init(&m);
    kv_mesh_generate(&m, blocks, 0, 0, 0, NULL);
...
Test(voxel, world_offset_translates_vertex_positions) {
    ...
    kv_mesh_generate(&m, blocks, 1, 0, 0, NULL);
...
Test(voxel, lod_step_one_matches_kv_mesh_generate) {
    ...
    kv_mesh_init(&a);
    kv_mesh_init(&b);
    kv_mesh_generate(&a, blocks, 0, 0, 0, NULL);
    kv_mesh_generate_lod(&b, blocks, 0, 0, 0, 1, NULL);
...
Test(voxel, lod_coarser_step_still_produces_geometry_for_solid_region) {
    ...
    KvMesh m;
    kv_mesh_init(&m);
    kv_mesh_generate_lod(&m, blocks, 0, 0, 0, 4, NULL);
